...
export const submitStatusApi = async (payload: {
  id: string;
  status: string;
}): Promise<IUser> => {
  try {
    ...
    const body = { status };
    const response: AxiosResponse<IUser> = await axiosInstance.put(
      `/api/user/status/${id}`,
      body
    );
    return response.data;
...
export const requestConnectionApi = async (payload: {
  inviterId: string;
  inviteeId: string;
}): Promise<IUser> => {
  try {
...
export const signUserApi = async (data: {
  username?: string;
  email?: string;
  imgUrl?: string;
}): Promise<IUser> => {
